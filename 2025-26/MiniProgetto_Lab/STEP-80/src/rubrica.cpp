...
int GestoreRAMOrdinata::compare_cognome_nome(const Contatto* a, const Contatto* b) const {
...
void GestoreRAMOrdinata::copy_contatto(Contatto* target, const Contatto* src) {
    if (!target || !src) {
        return;
    }
...
void GestoreRAMOrdinata::init_demo(int n){
    struct DemoEntry {
        const char* nome;
        const char* cognome;
        const char* telefono;
...
int GestoreRAMOrdinata::add_unsorted(const Contatto* c) {
...
int GestoreRAMOrdinata::add_ordered(const Contatto* c) {
    if (!c || rubrica_size >= MAX_CONTATTI) {
        return 0;
    }
    int pos = 0;
...
int GestoreMirroredRAM::compare_cognome(const Contatto* a, const Contatto* b) const {
    return std::strcmp(a->getCognome(), b->getCognome());
}

int GestoreMirroredRAM::compare_cognome_nome(const Contatto* a, const Contatto* b) const {
    int cmp = std::strcmp(a->getCognome(), b->getCognome());
    if (cmp != 0) {
        return cmp;
    }
...
void GestoreMirroredRAM::copy_contatto(Contatto* target, const Contatto* src) {
    if (!target || !src) {
        return;
    }
    target->init(src->getNome(), src->getCognome(), src->getTelefono());
...
int GestoreMirroredRAM::load_from_file() {
    if (!file_path) {
        return 0;
    }
    ...
    while (rubrica_size < MAX_CONTATTI) {
        int res = std::fscanf(f, " %31[^;];%31[^;];%15[^\n]\n", nome, cognome, telefono);
        if (res != 3) {
            break;
        }
        Contatto c;
...
void GestoreMirroredRAM::save_to_file() const {
    if (!file_path) {
        return;
    }
    ...
    for (int i = 0; i < rubrica_size; ++i) {
        std::fprintf(f, "%s;%s;%s\n", rubrica[i].getNome(), rubrica[i].getCognome(), rubrica[i].getTelefono());
    }
    std::fclose(f);
}

GestoreMirroredRAM::GestoreMirroredRAM(const char* path)
    : rubrica_size(0), sorted_state(1), file_path(path ? path : "rubrica.txt") {
    if (!load_from_file()) {
        init_demo(10); 
        save_to_file();
    }
...
void GestoreMirroredRAM::init_demo(int n){
    struct DemoEntry {
        const char* nome;
        const char* cognome;
        const char* telefono;
...
int GestoreMirroredRAM::add_unsorted(const Contatto* c) {
    if (!c || rubrica_size >= MAX_CONTATTI) {
        return 0;
    }
    copy_contatto(&rubrica[rubrica_size], c);
...
int GestoreMirroredRAM::add_ordered(const Contatto* c) {
    if (!c || rubrica_size >= MAX_CONTATTI) {
        return 0;
    }
    int pos = 0;
...
void GestoreMirroredRAM::lista() const {
    if (rubrica_size == 0) {
        std::cout << "Rubrica vuota." << std::endl;
        return;
    }
...
void GestoreMirroredRAM::print_at(int idx) const {
    if (idx < 0 || idx >= rubrica_size) {
        return;
    }
    rubrica[idx].print();
...
void GestoreMirroredRAM::sort() {
    if (sorted_state) return;
    for (int i = 1; i < rubrica_size; ++i) {
        Contatto key = rubrica[i];
        int j = i - 1;
        while (j >= 0 && rubrica[j].compare(key) > 0) {
...
bool GestoreMirroredRAM::is_sorted() {
    for (int i = 1; i < rubrica_size; ++i) {
        if (compare_cognome_nome(&rubrica[i-1], &rubrica[i]) > 0) {
            sorted_state = 0;
            return false;
        }
    }
...
int GestoreMirroredRAM::find_sequential(const char* cognome, const char* nome, int use_nome) const {
    if (!cognome) {
        return -1;
    }
    for (int i = 0; i < rubrica_size; ++i) {
...
int GestoreMirroredRAM::find_binary(const char* cognome, const char* nome, int use_nome) const {
    if (!cognome) {
        return -1;
    }
    Contatto key;
